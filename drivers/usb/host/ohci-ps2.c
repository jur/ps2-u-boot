/*
 * URB OHCI HCD (Host Controller Driver) for USB on the Sony Playstation 2.
 *
...
#define BUS2VIRT(x) ((x) == 0 ? 0 : KSEG1ADDR(ps2sif_bustophys(x)))
#define VIRT2BUS(x) ((x) == 0 ? 0 : ps2sif_phystobus(virt_to_phys(x)))
...
void ep_print_int_eds(ohci_t *ohci, char *str)
{
	...
	for (i = 0; i < 32; i++) {
		...
		while (*ed_p != 0 && j--) {
			ed_t *ed = (ed_t *)BUS2VIRT(m32_swap(*ed_p));
...
static int ep_link(ohci_t *ohci, ed_t *edi)
{
	...
	case PIPE_CONTROL:
		...
		if (ohci->ed_controltail == NULL)
			ohci_writel(VIRT2BUS(ed), &ohci->regs->ed_controlhead);
		else
			ohci->ed_controltail->hwNextED =
						   m32_swap((unsigned long)VIRT2BUS(ed));
	...
	case PIPE_BULK:
		...
		if (ohci->ed_bulktail == NULL)
			ohci_writel(VIRT2BUS(ed), &ohci->regs->ed_bulkhead);
		else
	...
	case PIPE_INTERRUPT:
		...
		for (i = 0; i < ep_rev(6, interval); i += inter) {
			...
			for (ed_p = &(ohci->hcca->int_table[\
				...
				(*ed_p != 0) &&
				(((ed_t *)BUS2VIRT(*ed_p))->int_interval >= interval);
				ed_p = &(((ed_t *)BUS2VIRT(*ed_p))->hwNextED))
					inter = ep_rev(6,
						 ((ed_t *)BUS2VIRT(*ed_p))->int_interval);
			ed->hwNextED = *ed_p;
			*ed_p = m32_swap((unsigned long)VIRT2BUS(ed));
		}
		break;
	}
	return 0;
			    ...
			    unsigned index, unsigned period)
{
	for (; index < NUM_INTS; index += period) {
		...
		while (*ed_p != 0) {
			if (((struct ed *)
					BUS2VIRT(m32_swap((unsigned long)*ed_p))) == ed) {
				*ed_p = ed->hwNextED;
				break;
			}
			ed_p = &(((struct ed *)
				     BUS2VIRT(m32_swap((unsigned long)*ed_p)))->hwNextED);
		}
	}
}
			...
			int interval, int load)
{
	...
	if (ed->state == ED_NEW) {
		...
		td = td_alloc(usb_dev);
		ed->hwTailP = m32_swap((unsigned long)VIRT2BUS(td));
	...
	struct usb_device *dev, int index, urb_priv_t *urb_priv)
{
	...
	td = urb_priv->td [index] =
			     (td_t *)(BUS2VIRT(m32_swap(urb_priv->ed->hwTailP) & ~0xf));
	...
	if (data) {
		/* Normal memory can't be used for the DMA transfer,
		 * we need to allocate DMA capable memory and copy the data to
		 * it.
		 */
		/* TBD: ps2sif_allociopheap() is a little bit slow. */
		td->dmamem = ps2sif_allociopheap(len);
		assert(td->dmamem != 0);
		if (td->dmamem != 0) {
			void *dst;

			dst = BUS2VIRT(td->dmamem);
			memcpy(dst, data, len);
		}
		td->hwBE = m32_swap((unsigned long)(td->dmamem + len - 1));
	} else {
		td->hwBE = 0;
		td->dmamem = 0;
	}
	td->hwCBP = m32_swap((unsigned long)td->dmamem);

	td->hwNextTD = m32_swap((unsigned long)VIRT2BUS(td_pt));
	/* append to queue */
...
static void dl_transfer_length(td_t *td)
{
	...
	if (!(usb_pipecontrol(lurb_priv->pipe) &&
		...
		if (tdBE != 0) {
			...
			if (td->hwCBP == 0)
				len = tdBE - td->dmamem + 1;
			else
				len = tdCBP - td->dmamem;
			lurb_priv->actual_length += len;
			/* Copy received data to the buffer. */
			memcpy(td->data,
				BUS2VIRT(td->dmamem), len);
		}
	}
	if (td->dmamem != 0) {
		/* Need to free memory allocated for DMA transfer. */
		ps2sif_freeiopheap(td->dmamem);
	}
	td->dmamem = 0;
}
...
static td_t *dl_reverse_done_list(ohci_t *ohci)
{
	...
	while (td_list_hc) {
		td_list = (td_t *)BUS2VIRT(td_list_hc);
...
int usb_lowlevel_init(int index, enum usb_init_type init, void **controller)
{
	...
	ghcca = ps2sif_allociopheap(sizeof(ed_t) * NUM_EDS);
	if (ghcca == 0) {
		err("Failed to allocate HCCA!!");
		return -1;
	}
	phcca = BUS2VIRT(ghcca);
	...
	ged = ps2sif_allociopheap(sizeof(ed_t) * NUM_EDS);
	if (ged == 0) {
		err("Failed to allocate EDs!!");
		return -1;
	}
	if (ged & 0x7) {
		err("EDs not aligned!!");
	...
	ohci_dev.ed = (void *) BUS2VIRT(ged);
	memset(ohci_dev.ed, 0, sizeof(ed_t) * NUM_EDS);
	gtd = ps2sif_allociopheap(sizeof(td_t) * NUM_TD);
	if (gtd == 0) {
		err("Failed to allocate TDs!!");
		return -1;
	}
	if ((__u32)gtd & 0x7) {
		err("TDs not aligned!!");
...
int usb_lowlevel_stop(int index)
{
	...
	if (usb_cpu_stop())
		return -1;
#endif
	/* Need to free allocated IOP memory, because the memory will be needed
	 * by Linux.
	 */
	ps2sif_freeiopheap(ghcca);
	ps2sif_freeiopheap(ged);
	ps2sif_freeiopheap(gtd);
